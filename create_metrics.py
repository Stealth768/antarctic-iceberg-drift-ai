...
def calculate_trajectory_metrics(df_sim, df_truth, time_col="timestamp", lat_col="latitude", lon_col="longitude"):
    merged = pd.merge(
    ...
    lat_sim, lon_sim = merged[f"{lat_col}_sim"].values, merged[f"{lon_col}_sim"].values
    lat_gt, lon_gt = merged[f"{lat_col}_truth"].values, merged[f"{lon_col}_truth"].values
    
    step_errors = haversine_distance(lat_sim, lon_sim, lat_gt, lon_gt)
    ...
    if len(merged) > 1:
        geod = pyproj.Geod(ellps="WGS84")
        az_sim, _, _ = geod.inv(lon_sim[:-1], lat_sim[:-1], lon_sim[1:], lat_sim[1:])
        az_gt, _, _ = geod.inv(lon_gt[:-1], lat_gt[:-1], lon_gt[1:], lat_gt[1:])
...
(metrics_dir / 'trajectory.py').write_text(code, encoding='utf-8')
print("PASS: Created src/metrics/trajectory.py successfully!")
